...
const EVOLUTION = [
  {
    title: 'Training, not prediction',
    text: 'The model is useful for comparing options and understanding fire behaviour. It is not calibrated to certify a real incident outcome.',
  },
  {
    title: 'Coverage is still uneven',
    text: 'Terrain, road and exposure detail is strongest in the areas currently represented by the project. Other exercise areas use a lighter landscape model.',
  },
  {
    title: 'One shared operational picture',
    text: 'The next step is a clearer handover between roles: field observations, planning assumptions and command decisions should stay attached to the same incident view.',
  },
  {
    title: 'Coordination that stays accountable',
    text: 'Future collaboration can add team notes, role-based reviews and a traceable decision history while keeping the incident commander in control.',
  },
];
      ...
      <main id="content">
        ...
        <section id="validation" className="landing-section landing-section-alt">
          ...
          <div className="ecarts" data-reveal>
            {EVOLUTION.map((item) => (
              <article key={item.title} className="ecart-card glass-panel evolution-card">
                <h3>{item.title}</h3>
                <p>{item.text}</p>
              </article>
            ))}
