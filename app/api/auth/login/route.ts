...
export async function POST(request: Request) {
  ...
  if (!limit.allowed) {
    return NextResponse.json({ error: 'Trop de tentatives. Réessayez plus tard.', retryAfterSeconds: limit.retryAfterSeconds }, {
      status: 429,
  ...
  if (!user || !valid) {
    ...
    return NextResponse.json({ error: 'Identifiants invalides.' }, { status: 401, headers: { 'Cache-Control': 'no-store' } });
  }
