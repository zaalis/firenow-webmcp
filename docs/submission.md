...
Every one of those routes runs the same `callTool`, with the same validation, the same log and the same single human approval on `commit_plan`. None of them is addressed to a human: the page carries no tool panel, no MCP catalogue and no manual call form. What an operator sees is the map; what an agent sees is the tools; and the WebMCP log shows the calls that actually ran.
