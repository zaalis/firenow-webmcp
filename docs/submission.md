...
Where the browser supplies `document.modelContext` natively — ChatGPT's built-in browser does — the page registers its tools on it with `registerTool()` in the top-level document, and they are discovered as site tools. Nowhere else does the page get in the way.

Because that API does not yet exist in a mainstream browser, the page also provides its own model context when the browser has none — without ever replacing a native implementation, and without squatting the namespace one will use: the property it defines is writable, and its setter adopts a native context that arrives later, carrying the already-registered tools across. The tools are therefore genuinely callable today, in Chrome: from a native WebMCP client, from an agent that evaluates JavaScript in the tab (`window.__WEBMCP__.callTool`), and from an extension content script — which runs in an isolated world and reaches neither of those — over a same-origin `postMessage` channel or a `webmcp:call` DOM event.

All of those are JavaScript, and the agent most people will actually point at this page may have none of them: an agent that drives a tab through screenshots and the accessibility tree reads "21 WebMCP tools" in the header and can call nothing. So the tools are also published **in the DOM**, in an agent bridge under the header: the directive, the catalogue with signatures and descriptions, a form that takes a tool name and JSON arguments, and a result pane — plus invocation by navigation, `/?tool=NAME&args=JSON`, for an agent whose only verb is opening a URL. Every route runs the same `callTool`, with the same validation, the same log and the same single human approval on `commit_plan`, which is the one tool the URL route refuses.
